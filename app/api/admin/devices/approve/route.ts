...
async function approveByPairCode(req: NextRequest) {
  ...
  // Accept either query string (?pair_code=123) or JSON body { pair_code: "123" }
  ...
  const deviceToken = `IMAOS_DEV_${crypto.randomUUID()}`;

  const { data, error } = await (supabase as any)
    .from("devices") // IMPORTANT: using devices (not devices_v2)
    .update({
      status: "active",
      approved_at: new Date().toISOString(),
      device_token: deviceToken, // overwrite/issue token on approve
    })
...
// Allow BOTH methods so the UI can’t 405 you.
export async function POST(req: NextRequest) {
  return approveByPairCode(req);
}
